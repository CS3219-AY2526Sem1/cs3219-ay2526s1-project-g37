...
// import * as jwt from 'lib0/crypto/jwt'  // Comment out
// import * as ecdsa from 'lib0/crypto/ecdsa'  // Comment out
...
export const createYWebsocketServer = async ({
  ...
  // Replace the entire auth callback with a simple one
  await registerYWebsocketServer(app, '/:room', store, async (req) => {
    const room = req.getParameter(0)
    
    // OPTION 1: No auth at all - just allow everything
    return { 
      hasWriteAccess: true, 
      room, 
      userid: 'anonymous' 
