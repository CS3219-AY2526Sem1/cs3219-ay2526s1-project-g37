...
def test_create_question_success_without_images(mock_db_conn):
    """
    ...
    # Check DB calls
    assert mock_db_conn.execute.call_count == 1 # Only question inserted


# Image upload is handled via /images, not question creation


def test_create_question_db_fails(mock_db_conn):
    mock_db_conn.execute.side_effect = Exception("DB Error")
    with pytest.raises(Exception, match="DB Error"):
        crud.create_question("Test Q", "Desc", "Easy", "Math")

def test_get_question_success(mock_db_conn):
    """Tests successful retrieval of a question."""
    # Arrange
    qid = str(uuid.uuid4())
...
def test_get_random_question_success(mock_db_conn):
...
def test_delete_question_success(mock_db_conn):
...
def test_successful_override(mock_question_funcs, backup_data):
    ...
    mock_create.assert_called_once_with("New", "New desc", "Hard", "Science", "qid123")
